...
class SimRealComparison:
    ...
    def load_real_data(self, filename):
        ...
        # Use real data as is (no transformations)
        position_data = df_clean['position_rad'].values
        velocity_data = df_clean['velocity_rad_s'].values
        acceleration_data = df_clean['acceleration_rad_s2'].values

        print(f"\n📐 Real data (no transformations):")
...
def main():
    ...
    print("="*60)
    print("SIMULATION VS REAL COMPARISON - KNEE LINK (RADIANS)")
    print("="*60)
    print()
